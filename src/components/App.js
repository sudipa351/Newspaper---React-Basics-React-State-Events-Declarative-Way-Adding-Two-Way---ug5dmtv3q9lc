...
const App = () => {
  ...
  useEffect(() => {
    if (lat !== "" && long !== "") {
      ...
      fetch(
        `http://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&appid=${"da05b6e0fe788db7dcdc421c300214c1"}`,
        { method: "GET" }
      )
        .then((e) => e.json())
        .then((e) => {
          setWeather(
            JSON.stringify({ temperature: e.main.temp, location: e.coord })
          );
        });
      fetch(
        `https://gnews.io/api/v4/search?q=example&token=410db42779f25b2d81028050efe65502&max=5`
      )
        .then((e) => e.json())
        .then((e) => {
          // for(int )
  ...
  const findFromCity = (e) => {
    fetch(
      `http://api.openweathermap.org/data/2.5/weather?q=${e}&appid=${"da05b6e0fe788db7dcdc421c300214c1"}`,
      { method: "GET" }
    )
      .then((e) => e.json())
      .then((e) => {
